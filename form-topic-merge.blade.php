...
<div id="bbpress-forums" class="bbpress-wrapper">
    ...
    @if ( is_user_logged_in() && current_user_can( 'edit_topic', bbp_get_topic_id() ) )
        ...
        <div id="merge-topic-@php bbp_topic_id(); @endphp" class="bbp-topic-merge">

            <form id="merge_topic" name="merge_topic" method="post">

                <fieldset class="bbp-form">

                    <legend>@php printf( esc_html__( 'Merge topic "%s"', 'bbpress' ), bbp_get_topic_title() ); @endphp</legend>

                    <div>

                        <div class="bbp-template-notice info">
                            <ul>
                                <li>@php esc_html_e( 'Select the topic to merge this one into. The destination topic will remain the lead topic, and this one will change into a reply.', 'bbpress' ); @endphp</li>
                                <li>@php esc_html_e( 'To keep this topic as the lead, go to the other topic and use the merge tool from there instead.',                                  'bbpress' ); @endphp</li>
                            </ul>
                        </div>

                        <div class="bbp-template-notice">
                            <ul>
                                <li>@php esc_html_e( 'Replies to both topics are merged chronologically, ordered by the time and date they were published. Topics may be updated to a 1 second difference to maintain chronological order based on the merge direction.', 'bbpress' ); @endphp</li>
                            </ul>
                        </div>

                        <fieldset class="bbp-form">
                            <legend>@php esc_html_e( 'Destination', 'bbpress' ); @endphp</legend>
                            <div>
                                ...
                                @if ( bbp_has_topics( array( 'show_stickies' => false, 'post_parent' => bbp_get_topic_forum_id( bbp_get_topic_id() ), 'post__not_in' => array( bbp_get_topic_id() ) ) ) )
                                    ...
                                    <label for="bbp_destination_topic">@php esc_html_e( 'Merge with this topic:', 'bbpress' ); @endphp</label>
                                ...
                                @else
                                    ...
                                    <label>@php esc_html_e( 'There are no other topics in this forum to merge with.', 'bbpress' ); @endphp</label>
                        ...
                        <fieldset class="bbp-form">
                            ...
                            <div>
                                ...
                                <input name="bbp_topic_favoriters" id="bbp_topic_favoriters" type="checkbox" value="1"
                                       checked="checked"/>
                                <label for="bbp_topic_favoriters">@php esc_html_e( 'Merge topic favoriters', 'bbpress' ); @endphp</label><br/>
